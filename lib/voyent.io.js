...
(function(window) {
//"window" will be window or module.exports depending on if we loaded it in the
//browser or as a node module but we'll just name it window for simplicity's sake
    ...
    //these functions exist in the voyent.js library but we don't want that dependency so we'll
    //add them here with some extra conditionals for when using this lib as a node.js module
    var v = {
            ...
            if (!('bridgeit_useLocalStorage' in window)) {
                if ('localStorage' in window) {
                    try {
                        ...
                        window.bridgeit_useLocalStorage = localStorage.getItem('testdate') === testdate;
        ...
        getLocalStorageItem: function(key) {
            if (!v.io.isNode) {
                ...
                return v.getNodeStorageItem(key);
            }
        },
		setLocalStorageItem: function(key, value) {
            if (!v.io.isNode) {
                if (v.useLocalStorage()) {
                    ...
                    return v.setCookie(key, value);
                }
            }
            else {
                return v.setNodeStorageItem(key, value);
            }
		},
        removeLocalStorageItem: function(key) {
            if (!v.io.isNode) {
                if (v.useLocalStorage()) {
                    ...
                    v.setCookie(key, null);
                }
            }
            else {
                return v.removeNodeStorageItem(key);
            }
        },
        ...
        setSessionStorageItem: function (key, value) {
            if (!v.io.isNode) {
                if (v.useLocalStorage()) {
                    ...
                    return v.setCookie(key, value, 1);
                }
            }
            else {
                return v.setNodeStorageItem(key, value);
        ...
        getNodeStorageItem: function(key) {
            return typeof nodeStorage[key] !== 'undefined' ? nodeStorage[key] : null;
        },
        setNodeStorageItem: function(key, value) {
            nodeStorage[key] = value;
        },
        removeNodeStorageItem: function(key) {
            delete nodeStorage[key];
        },
        io: {
		    //take our best guess at detecting node
    ...
    if (v.io.isNode) {
        //lib loaded via node, export as node module
        module.exports = v.io;
    }
    else {
        //lib loaded in browser, export into the window object
        if (!window.voyent) {
            window.voyent = v;
        }
        window.voyent.io = v.io;
    }
	...
	function validateRequiredConfirmationId(params, reject) {
	    validateParameter('confirmationId', 'The confirmationId parameter is required', params, reject);
	}
	
	function validateRequiredMetadata(params, reject) {
	    validateParameter('metadata', 'The metadata parameter is required', params, reject);
	}
	
	function validateRequiredAccount(params, reject) {
	    validateParameter('account', 'The account parameter is required', params, reject);
	}
	...
	function getRealmResourceURL(servicePath, account, realm, resourcePath, token, ssl, params){
		...
		var url = determineProtocol(ssl) + servicePath +
	...
	v.io.configureHosts = function(url) {
		if (!url) {
			v.io.baseURL = v.io.isNode ? 'dev.voyent.cloud' : window.location.hostname;
		}
		else {
			v.io.baseURL = url;
		}
	...
	v.io.checkHost = function(params) {
		if (params.host) {
		    ...
		    var lastHost = v.io.auth.getLastKnownHost();
		    if (lastHost) {
		        v.io.configureHosts(lastHost);
		    }
		}
	};
	...
	v.io.admin = {
		...
		getServiceDefinitions: function(params){
			return new Promise(
				function(resolve, reject) {
					...
					var token = validateAndReturnRequiredAccessToken(params, reject);
					var txParam = getTransactionURLParam();
					var url = determineProtocol(params.ssl) + v.io.authAdminURL + '/system/services/?access_token=' + token +
						(txParam ? '&' + txParam : '');
		...
		getAccount: function(params){
			return new Promise(function(resolve, reject) {
				...
				var txParam = getTransactionURLParam();
				var url = determineProtocol(params.ssl) + v.io.authAdminURL + '/' + account + '?access_token=' + token +
					(txParam ? '&' + txParam : '');
		 ...
		 */
		 createAccount: function(params){
			return new Promise(function(resolve, reject) {
				...
				var txParam = getTransactionURLParam();
				var url = determineProtocol(params.ssl) + v.io.authAdminURL + '/' + (txParam ? '&' + txParam : '');
				...
				v.$.post(url, {account: account}).then(function(json){
					...
					if (params.host) {
					    v.setSessionStorageItem(btoa(HOST_KEY), btoa(params.host));
					}
		...
		confirmAccount: function(params) {
		    return new Promise(function(resolve, reject) {
				...
				var txParam = getTransactionURLParam();
				var url = determineProtocol(params.ssl) + v.io.authAdminURL + '/' + params.account + '/confirm' + (txParam ? '&' + txParam : '');
				v.$.post(url, {confirmationId: params.confirmationId}).then(function(json){
                    ...
                    if (json.username) {
                        v.setSessionStorageItem(btoa(USERNAME_KEY), btoa(json.username));
                    }
                    if (params.account) {
		...
		resendConfirmAccountEmail: function(params) {
		    return new Promise(function(resolve, reject) {
				...
				var txParam = getTransactionURLParam();
				var url = determineProtocol(params.ssl) + v.io.authAdminURL + '/' 
		...
		getRealms: function(params){
			return new Promise(function(resolve, reject) {
				...
				var txparam = getTransactionURLParam();
				var url = determineProtocol(params.ssl) + v.io.authAdminURL + '/' + account + '/realms/'
		...
		createRealm: function(params){
			return new Promise(function(resolve, reject) {
				...
				var txParam = getTransactionURLParam();
				var url = determineProtocol(params.ssl) + v.io.authAdminURL + '/' + account + '/realms?access_token=' + token +
					(txParam ? '&' + txParam : '');
		...
		cloneRealm: function(params){
			return new Promise(function(resolve, reject) {
				...
				var txParam = getTransactionURLParam();
				var url = determineProtocol(params.ssl) + v.io.authAdminURL + '/' + account + '/realms/' + originRealmName + '/clone/' +
        ...
        createAnonUser: function(params){
            return new Promise(function(resolve, reject) {
                ...
                var password = v.io.auth.generatePassword()
                ...
                v.$.post(url, toPost).then(function(response){
                    v.io.auth.updateLastActiveTimestamp();
                    var anonUser = {password:password,username:response.resourceLocation.substring( response.resourceLocation.lastIndexOf('/')+1)};
                    resolve(anonUser);
                })['catch'](function(error){
                    reject(error);
                });
		...
		resendConfirmRealmUserEmail: function(params) {
		    return new Promise(function(resolve, reject) {
                ...
                var token = validateAndReturnRequiredAccessToken(params, reject);
				var txParam = getTransactionURLParam();
				var url = determineProtocol(params.ssl) + v.io.authAdminURL + '/' + params.account
				                   + '/realms/' + params.realm
...
/* Realm Contexts */
      ...
      return new Promise(function(resolve, reject) {
		...
		getLogs: function(params) {
			return new Promise(function(resolve, reject) {
				...
				var url = determineProtocol(params.ssl) + v.io.authAdminURL + '/' + account + '/logging/?access_token=' +
		...
		getDebugLogs: function(params) {
			return new Promise(function(resolve, reject) {
				...
				var url = determineProtocol(params.ssl) + v.io.authAdminURL + '/' + account + '/realms/' + realm +
		...
		createAdministrator: function(params){
			return new Promise(function(resolve, reject) {
				...
				var txParam = getTransactionURLParam();
				var url = determineProtocol(params.ssl) + v.io.authAdminURL + '/' + account + '/admins/?access_token=' + token +
					(txParam ? '&' + txParam : '');
		...
		updateAdministrator: function(params){
			return new Promise(function(resolve, reject) {
				...
				var txParam = getTransactionURLParam();
				var url = determineProtocol(params.ssl) + v.io.authAdminURL + '/' + account + '/admins/' + admin.username +
		...
		deleteAdministrator: function(params){
			return new Promise(function(resolve, reject) {
				...
				var txParam = getTransactionURLParam();
				var url = determineProtocol(params.ssl) + v.io.authAdminURL + '/' + account + '/admins/' + username + '/?access_token=' + token +
					(txParam ? '&' + txParam : '');
	...
	v.io.auth = {
		 ...
		 getNewAccessToken: function(params){
			return new Promise(
				function(resolve, reject) {
					...
					var url = determineProtocol(params.ssl) + v.io.authURL + '/' + encodeURI(params.account) +
		...
		connect: function(params){
			return new Promise(function(resolve, reject) {
				...
				function saveCredentials(){
					...
					v.setSessionStorageItem(btoa(HOST_KEY), btoa(v.io.auth.getLastKnownHost()));
					v.setSessionStorageItem(btoa(USERNAME_KEY), btoa(params.username));
			...
			return new Promise(function(resolve, reject) {
				if( !v.io.auth.isLoggedIn()){
					...
					if( !loginParams ){
						...
						loginParams.host = atob(v.getSessionStorageItem(btoa(HOST_KEY)));
		...
		disconnect: function(){
			...
			v.removeSessionStorageItem(btoa(HOST_KEY));
		...
		isLoggedIn: function(){
			...
			var result = token && tokenExpiresIn && tokenSetAt && (new Date().getTime() < (tokenExpiresIn + tokenSetAt) ) && (v.io.isNode || (!v.io.isNode && (isDev || currentPath.indexOf(scopeToPath) === 0)));
			return !!result;
		},
		...
		getLastKnownHost: function () {
			var hostCipher = v.getSessionStorageItem(btoa(HOST_KEY));
			if (hostCipher) {
		...
		forgotPassword: function(params){
			return new Promise(function(resolve, reject) {
				...
				var txParam = getTransactionURLParam();
				var url = determineProtocol(params.ssl) + v.io.authAdminURL + '/' + account + '/';
