if( ! ('voyent' in window)){
	throw new Error('voyent-voyent.js requires voyent.js, please include voyent.js before voyent-voyent.js');
}
...
(function(b) {
	...
	function validateAndReturnRequiredAccessToken(params, reject){
		var token = params.accessToken || voyent.auth.getLastAccessToken();
		if( token ){
			return token;
		}
		else{
	...
	function validateAndReturnRequiredAccount(params, reject){
		...
		if( account ){
			...
			account = voyent.auth.getLastKnownAccount();
		}
		if( account ){
	...
	function getTransactionURLParam(){
		var txId = voyent.getLastTransactionId();
		if( txId ){
			return 'tx=' + txId;
		}
		else{
			...
			return new Promise(function(resolve, reject) {
				...
				if( progressCallback ){
					request.upload.addEventListener("progress", function(evt){
						voyent.auth.updateLastActiveTimestamp();
	...
	voyent.configureHosts = function(url){
		var isLocal = ['localhost','127.0.0.1'].indexOf(url) > -1;
		if( !url ){
			voyent.baseURL = 'dev.voyent.io';
		}
		else{
			voyent.baseURL = url;
		}
		var baseURL = voyent.baseURL;
		voyent.authURL = baseURL + (isLocal ? ':55010' : '') + '/auth';
		voyent.authAdminURL = baseURL + (isLocal ? ':55010' : '') + '/authadmin';
		voyent.locateURL = baseURL + (isLocal ? ':55020' : '') + '/locate';
		voyent.documentsURL = baseURL + (isLocal ? ':55080' : '') + '/docs';
		voyent.storageURL = baseURL + (isLocal ? ':55030' : '') + '/storage';
		voyent.metricsURL = baseURL + (isLocal ? ':55040' : '') + '/metrics';
		voyent.eventURL = baseURL + (isLocal ? ':55050' : '') + '/event';
		voyent.contextURL = baseURL + (isLocal ? ':55060' : '') + '/context';
		voyent.codeURL = baseURL + (isLocal ? ':55090' : '') + '/code';
		voyent.pushURL = baseURL + (isLocal ? ':8080' : '') + '/push';
		voyent.pushRESTURL = voyent.pushURL + '/rest';
		voyent.queryURL = baseURL + (isLocal ? ':55110' : '') + '/query';
		voyent.actionURL = baseURL + (isLocal ? ':55130' : '') + '/action';
		voyent.eventhubURL = baseURL + (isLocal ? ':55200' : '') + '/eventhub';
		voyent.mailboxURL = baseURL + (isLocal ? ':55120' : '') + '/mailbox';
		voyent.deviceURL = baseURL + (isLocal ? ':55160' : '') + '/device';
	};

	voyent.checkHost = function(params){
		//TODO use last configured host if available
		if( params.host ){
			voyent.configureHosts(params.host);
		}
	};

	voyent.startTransaction = function(){
		b.setSessionStorageItem(btoa(TRANSACTION_KEY), b.$.newUUID());
		console.log('voyent: started transaction ' + voyent.io.getLastTransactionId());
	};

	voyent.endTransaction = function(){
		b.removeSessionStorageItem(btoa(TRANSACTION_KEY));
		console.log('voyent: ended transaction ' + voyent.io.getLastTransactionId());
	};

	voyent.getLastTransactionId = function(){
		return b.getSessionStorageItem(btoa(TRANSACTION_KEY));
	};

	voyent.setCurrentRealm = function(realm){
		b.setSessionStorageItem(btoa(REALM_KEY), btoa(realm));
	};

	voyent.getResourcePermissions = function(params){
		return new Promise(
			function(resolve, reject) {
				params = params ? params : {};
				voyent.checkHost(params);
				...
				switch(params.service){
					case 'documents': serviceURL = voyent.documentsURL; break;
					case 'action': serviceURL = voyent.actionURL; break;
					case 'eventhub': serviceURL = voyent.eventhubURL; break;
					case 'query': serviceURL = voyent.queryURL; break;
					case 'storage': serviceURL = voyent.storageURL; break;
					case 'mailbox': serviceURL = voyent.mailboxURL; break;
					case 'location': serviceURL = voyent.locateURL; break;
				}
	...
	voyent.updateResourcePermissions = function(params){
		return new Promise(
			function(resolve, reject) {
				params = params ? params : {};
				voyent.checkHost(params);
	...
	voyent.action = {
		...
		executeAction: function(params){
			return new Promise(
				function(resolve, reject) {
					...
					var url = getRealmResourceURL(voyent.actionURL, account, realm,
		...
		getResourcePermissions: function(params){
			...
			params.path = 'actions';
			return voyent.getResourcePermissions(params);
		},

		updateResourcePermissions: function(params){
			params.service = 'action';
			params.path = 'actions';
			return voyent.updateResourcePermissions(params);
		},
	};
	...
	voyent.admin = {
		...
		getServiceDefinitions: function(params){
			return new Promise(
				function(resolve, reject) {
					...
					var txParam = getTransactionURLParam();
					var url = protocol + voyent.authAdminURL + '/system/voyent/?access_token=' + token +
						(txParam ? '&' + txParam : '');
		...
		getAccount: function(params){
			return new Promise(function(resolve, reject) {
				...
				var protocol = params.ssl ? 'https://' : 'http://';
				var txParam = getTransactionURLParam();
				var url = protocol + voyent.authAdminURL + '/' + account + '?access_token=' + token +
					(txParam ? '&' + txParam : '');
		 ...
		 */
		 createAccount: function(params){
			...
			return new Promise(function(resolve, reject) {
				...
				var protocol = params.ssl ? 'https://' : 'http://';
				var txParam = getTransactionURLParam();
				var url = protocol + voyent.authAdminURL + '/' + (txParam ? '&' + txParam : '');
		...
		getRealms: function(params){
			return new Promise(function(resolve, reject) {
				...
				var protocol = params.ssl ? 'https://' : 'http://';
				var txparam = getTransactionURLParam();
				var url = protocol + voyent.authAdminURL + '/' + account + '/realms/'
		...
		getRealm: function(params){
			return new Promise(function(resolve, reject) {
				...
				var url = getRealmResourceURL(voyent.authAdminURL, account, realm,
					'', token, params.ssl);
		...
		updateRealm: function(params){
			return new Promise(function(resolve, reject) {
				...
				var url = getRealmResourceURL(voyent.authAdminURL, account, params.realm.name,
					'', token, params.ssl);
		...
		createRealm: function(params){
			return new Promise(function(resolve, reject) {
				...
				var protocol = params.ssl ? 'https://' : 'http://';
				var txParam = getTransactionURLParam();
				var url = protocol + voyent.authAdminURL + '/' + account + '/realms?access_token=' + token +
					(txParam ? '&' + txParam : '');
		...
		cloneRealm: function(params){
			return new Promise(function(resolve, reject) {
				...
				var protocol = params.ssl ? 'https://' : 'http://';
				var txParam = getTransactionURLParam();
				var url = protocol + voyent.authAdminURL + '/' + account + '/realms/' + originRealmName + '/clone/' +
		...
		deleteRealm: function(params){
			return new Promise(function(resolve, reject) {
				...
				var url = getRealmResourceURL(voyent.authAdminURL, account, realmName,
					'', token, params.ssl);
		...
		getLogs: function(params) {
			return new Promise(function(resolve, reject) {
				...
				var url = protocol + voyent.authAdminURL + '/' + account + '/logging/?access_token=' +
		...
		getDebugLogs: function(params) {
			return new Promise(function(resolve, reject) {
				...
				var url = protocol + voyent.authAdminURL + '/' + account + '/realms/' + realm +
		...
		createAdministrator: function(params){
			return new Promise(function(resolve, reject) {
				...
				var protocol = params.ssl ? 'https://' : 'http://';
				var txParam = getTransactionURLParam();
				var url = protocol + voyent.authAdminURL + '/' + account + '/admins/?access_token=' + token +
					(txParam ? '&' + txParam : '');
		...
		updateAdministrator: function(params){
			return new Promise(function(resolve, reject) {
				...
				var protocol = params.ssl ? 'https://' : 'http://';
				var txParam = getTransactionURLParam();
				var url = protocol + voyent.authAdminURL + '/' + account + '/admins/' + admin.username +
		...
		deleteAdministrator: function(params){
			return new Promise(function(resolve, reject) {
				...
				var protocol = params.ssl ? 'https://' : 'http://';
				var txParam = getTransactionURLParam();
				var url = protocol + voyent.authAdminURL + '/' + account + '/admins/' + username + '/?access_token=' + token +
					(txParam ? '&' + txParam : '');
	...
	voyent.auth = {
		 ...
		 getNewAccessToken: function(params){
			return new Promise(
				function(resolve, reject) {
					...
					var protocol = params.ssl ? 'https://' : 'http://';
					var url = protocol + voyent.authURL + '/' + encodeURI(params.account) +
		 ...
		 * @param {String} params.host The Voyent Services host url, defaults to api.voyent.io
		...
		connect: function(params){
			return new Promise(function(resolve, reject) {
				...
				function connectCallback(){
					...
					var inactiveMillis = now - voyent.auth.getLastActiveTimestamp();
					var millisUntilTimeoutExpires = timeoutMillis - inactiveMillis;
					console.log('voyent.io.auth.connect: getLastActiveTimestamp: ' + voyent.auth.getLastActiveTimestamp());
					...
					if( millisUntilTimeoutExpires > 0 ){
						console.log(new Date().toISOString() + ' voyent.io.auth.connect: timeout has not been exceeded, ' +
							voyent.auth.getTimeRemainingBeforeExpiry()/1000/60 + ' mins remaining before token expires, ' +
						...
						if( timeoutMillis > millisUntilTimeoutExpires ){
							voyent.auth.refreshAccessToken().then(function(){
								var cbId = setTimeout(connectCallback, voyent.auth.getExpiresIn() - timeoutPadding);
								b.setSessionStorageItem(btoa(RELOGIN_CB_KEY), cbId);
								console.log( new Date().toISOString() + ' voyent.io.auth.connect: setting next connection check to ' + voyent.auth.getExpiresIn() / 1000 / 60 + ' mins, expiresIn: ' +
									(voyent.auth.getExpiresIn()/1000/60) + ' mins, remaining: '  +
									(voyent.auth.getTimeRemainingBeforeExpiry()/1000/60) + ' mins');
						...
						if( expiredCallback ){
							...
							if( expiredCallbackFunction ){
								...
								if( expiredCallbackPromise && expiredCallbackPromise.then ){
									expiredCallbackPromise.then(voyent.auth.disconnect)
									['catch'](voyent.auth.disconnect);
								}
								else{
									setTimeout(voyent.auth.disconnect, 500);
								}
							}
							else{
				...
				function initConnectCallback(){
					...
					var callbackTimeout;
					if( connectionTimeoutMillis > voyent.auth.getTimeRemainingBeforeExpiry()){
						callbackTimeout = voyent.auth.getTimeRemainingBeforeExpiry() - timeoutPadding;
					}
					...
					tokenSetAt.setTime(voyent.auth.getTokenSetAtTime());
					console.log( new Date().toISOString() + ' voyent.io.auth.connect: setting next connection check to ' + callbackTimeout / 1000 / 60 + ' mins, expiresIn: ' +
						(voyent.auth.getExpiresIn()/1000/60) + ' mins, remaining: '  +
						(voyent.auth.getTimeRemainingBeforeExpiry()/1000/60) + ' mins, token set at ' + tokenSetAt);
					var cbId = setTimeout(connectCallback, callbackTimeout);
					b.setSessionStorageItem(btoa(RELOGIN_CB_KEY), cbId);

					if( settings.usePushService ){
						voyent.push.startPushService(settings);
					}
				}
				...
				function saveCredentials(){
					b.setSessionStorageItem(btoa(ACCOUNT_KEY), btoa(voyent.io.auth.getLastKnownAccount()));
					b.setSessionStorageItem(btoa(REALM_KEY), btoa(voyent.io.auth.getLastKnownRealm()));
					b.setSessionStorageItem(btoa(USERNAME_KEY), btoa(params.username));
				...
				if( voyent.auth.isLoggedIn()){
					...
					voyent.auth.login(params).then(function(authResponse){
		...
		refreshAccessToken: function(){
			return new Promise(function(resolve, reject) {
				if( !voyent.auth.isLoggedIn()){
					...
					var loginParams = voyent.auth.getConnectSettings();
					if( !loginParams ){
						...
						voyent.auth.login(loginParams).then(function(authResponse){
							fireEvent(window, 'voyent-access-token-refreshed', voyent.auth.getLastAccessToken());
							if( loginParams.usePushService ){
								voyent.push.startPushService(loginParams);
							}
							resolve(authResponse);
						})['catch'](function(response){
							reject(response);
		...
		getTimeRemainingBeforeExpiry: function(){
			var expiresIn = voyent.auth.getExpiresIn();
			var token = voyent.auth.getExpiresIn();
			if( expiresIn && token ){
				var now = new Date().getTime();
				return (voyent.auth.getTokenSetAtTime() + expiresIn) - now;
			}
		},
		...
		isLoggedIn: function(){
			...
			//console.log('voyent.io.auth.isLoggedIn: token=' + token + ' tokenExpiresIn=' + tokenExpiresIn + 'tokenSetAt=' + tokenSetAt + ' (new Date().getTime() < (tokenExpiresIn + tokenSetAt))=' + (new Date().getTime() < (tokenExpiresIn + tokenSetAt)) + ' (currentPath.indexOf(scopeToPath) === 0)=' + (currentPath.indexOf(scopeToPath) === 0));
		...
		forgotPassword: function(params){
			return new Promise(function(resolve, reject) {
				...
				var protocol = params.ssl ? 'https://' : 'http://';
				var txParam = getTransactionURLParam();
				var url = protocol + voyent.authAdminURL + '/' + account + '/';
		...
		createDocument: function(params){
			return new Promise(
				function(resolve, reject) {
					...
					var url = getRealmResourceURL(voyent.documentsURL, account, realm,
...
/**
    ...
    getCollections:function(params){
      return new Promise(
        function(resolve, reject) {
          ...
          b.$.getJSON(url).then(function(collections){
            voyent.auth.updateLastActiveTimestamp();
            resolve(collections);
          })['catch'](function(response){
	...
	voyent.eventhub = {
		...
		createHandler: function(params){
			return new Promise(
				function(resolve, reject) {
					...
					var url = getRealmResourceURL(voyent.eventhubURL, account, realm,
		...
		getRecognizerResourcePermissions: function(params){
			...
			return voyent.eventhub.getResourcePermissions(params);
		},
		...
		updateHandlerResourcePermissions: function(params){
			params.path = 'handlers';
			return voyent.eventhub.updateResourcePermissions(params);
		},
	...
	voyent.location = {
		 ...
		 createRegion: function(params){
			return new Promise(
				function(resolve, reject) {
					...
					var url = getRealmResourceURL(voyent.locateURL, account, realm,
		 ...
		 * @returns {Object} The single result, if any, of the user location.
		 ...
		 http://dev.voyent.io/locate/bsrtests/realms/test/locations
		...
		getRegionResourcePermissions: function(params){
			params.path = 'regions';
			return voyent.location.getResourcePermissions(params);
		},
		...
		updatePOIResourcePermissions: function(params){
			params.path = 'poi';
			return voyent.location.updateResourcePermissions(params);
		},
	...
	voyent.mailbox = {
		...
		createMailbox: function (params) {
			return new Promise(
				function (resolve, reject) {
					...
					var url = getRealmResourceURL(voyent.mailboxURL, account, realm,
		...
		getMailboxResourcePermissions: function(params){
			...
			return voyent.mailbox.getResourcePermissions(params);
		},
		...
		findEvents: function(params){
			return new Promise(
				function(resolve, reject) {
					...
					var url = getRealmResourceURL(voyent.eventURL, account, realm,
...
/**
    ...
    createCustomEvents: function(params){
      return new Promise(
        function(resolve, reject) {
          ...
          runningIndex += 1;
          eventIndex=0;
          voyent.event._eventRecursion(resolve,reject,url,params.ssl,runningIndex);
        }
      );
    },
    /**
    ...
    _eventRecursion: function(resolve,reject,url,ssl,index){
		...
		if (!url) {
			url = getRealmResourceURL(voyent.eventURL, voyent.auth.getLastKnownAccount(), voyent.auth.getLastKnownRealm(),
				'events', voyent.auth.getLastAccessToken(), !!ssl);
		}
            ...
            if(index === runningIndex) {
              ...
              b.$.post(url, eventArray[eventIndex]).then(function (response) {
                ...
                if (eventIndex === eventArray.length - 1) {
                  ...
                  eventIndex += 1;
                  voyent.event._eventRecursion(resolve, reject, null, ssl, index);
                }
              })['catch'](function (error) {
                reject(error);
              });
		...
		startPushService: function(params){
			return new Promise(
				function(resolve, reject) {
					...
					var pushURL = (params.ssl ? 'https://' : 'http://') + voyent.pushURL + '/';
					...
					b.$.get(pushURL + 'code.icepush').then(function(response){
						try{
							...
							ice.push.configuration.account = voyent.io.auth.getLastKnownAccount();
							ice.push.configuration.realm = voyent.io.auth.getLastKnownRealm();
							ice.push.configuration.access_token = voyent.io.auth.getLastAccessToken();
							...
							console.log('voyent.io.push.connect() failed: ' + e);
							reject(e);
						}
					})['catch'](function(error){
						reject(error);
			...
			return new Promise(
				function(resolve, reject) {
					...
					if (!absoluteGoVoyentURL)  {
						if (!!voyent.goVoyentURL)  {
							absoluteGoVoyentURL = getAbsoluteURL(voyent.goVoyentURL);
						}
					}
					if (!!absoluteGoVoyentURL)  {
		...
		getUser: function(params){
			return new Promise(
				function(resolve, reject) {
					...
					var url = getRealmResourceURL(voyent.contextURL, account, realm,
						'users/' + username, token, params.ssl);
	...
	voyent.storage = {
		...
		getMetaInfo: function(params){
			return new Promise(function(resolve, reject) {
				...
				var url = getRealmResourceURL(voyent.storageURL, account, realm,
		...
		getBlobResourcePermissions: function(params){
			params.path = 'blobs';
			return voyent.storage.getResourcePermissions(params);
		},
	...
	voyent.query = {
		...
		createQuery: function(params){
			return new Promise(
				function(resolve, reject) {
					...
					var url = getRealmResourceURL(voyent.queryURL, account, realm,
		...
		getQueryResourcePermissions: function(params){
			params.path = 'queries';
			return voyent.query.getResourcePermissions(params);
		},
	...
	voyent.device = {
		...
		startDevice: function (params) {
			return new Promise(function (resolve, reject) {
				...
				var url = getRealmResourceURL(voyent.deviceURL, account, realm,
		...
		getRunningDevices: function (params) {
			return new Promise(function (resolve, reject) {
				...
				var realm = validateAndReturnRequiredRealm(params, reject);
				var token = validateAndReturnRequiredAccessToken(params, reject);
				var url = getRealmResourceURL(voyent.deviceURL, account, realm, 'running', token, params.ssl);
