if( ! ('voyent' in window)){
	throw new Error('voyent-services.js requires voyent.js, please include voyent.js before voyent-services.js');
}
...
(function(b) {
	...
	services.configureHosts = function(url){
		...
		if( !url ){
			services.baseURL = 'dev.voyent.io';
		}
		else{
		...
		connect: function(params){
			return new Promise(function(resolve, reject) {
				...
				function connectCallback(){
					...
					console.log('voyent.io.auth.connect: getLastActiveTimestamp: ' + services.auth.getLastActiveTimestamp());
					...
					if( millisUntilTimeoutExpires > 0 ){
						...
						if( timeoutMillis > millisUntilTimeoutExpires ){
							services.auth.refreshAccessToken().then(function(){
								...
								console.log( new Date().toISOString() + ' voyent.io.auth.connect: setting next connection check to ' + services.auth.getExpiresIn() / 1000 / 60 + ' mins, expiresIn: ' +
				...
				if( !services.auth.isLoggedIn()){
					...
					if( !loginParams ){
						...
						services.auth.login(loginParams).then(function(authResponse){
							fireEvent(window, 'voyent-access-token-refreshed', services.auth.getLastAccessToken());
