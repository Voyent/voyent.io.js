		 ...
		 createAccount: function(params){
			...
			return new Promise(function(resolve, reject) {
				...
				b.$.post(url, {account: account}).then(function(json){
					...
					setSessionStorageItem(btoa(TOKEN_KEY), json.token.access_token);
					setSessionStorageItem(btoa(TOKEN_EXPIRES_KEY), json.token.expires_in);
					setSessionStorageItem(btoa(TOKEN_SET_KEY), loggedInAt);
					setSessionStorageItem(btoa(ACCOUNT_KEY), btoa(accountname));
					setSessionStorageItem(btoa(USERNAME_KEY), btoa(username));
					setSessionStorageItem(btoa(ADMIN_KEY), btoa('true'));
	...
	services.auth = {
			...
			return new Promise(function(resolve, reject) {
				...
				b.$.post(url, {
					...
					setSessionStorageItem(btoa(TOKEN_KEY), authResponse.access_token);
						...
						setSessionStorageItem(btoa(ADMIN_KEY), btoa(params.admin));
					if( params.scopeToPath ){
						setSessionStorageItem(btoa(SCOPE_TO_PATH_KEY), btoa(params.scopeToPath));	
					}
		...
		connect: function(params){
			return new Promise(function(resolve, reject) {
				...
				function initConnectCallback(){
					...
					console.log( new Date().toISOString() + ' bridgeit connect: setting next connection check to ' + callbackTimeout / 1000 / 60 + ' mins, expiresIn: ' + 
			...
			return new Promise(function(resolve, reject) {
				if( !services.auth.isLoggedIn()){
					...
					if( !loginParams ){
						...
						loginParams.admin = atob(getSessionStorageItem(btoa(ADMIN_KEY)));
		...
		isLoggedIn: function(){
			var token = getSessionStorageItem(btoa(TOKEN_KEY)),
				...
				scopeToPathCipher = getSessionStorageItem(btoa(SCOPE_TO_PATH_KEY)),
