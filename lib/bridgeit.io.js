	...
	services.admin = {
		...
		updateRealm: function(params){
			return new Promise(function(resolve, reject) {
				...
				b.$.put(url, {realm: params.realm}).then(function(json){
					services.auth.updateLastActiveTimestamp();
					resolve(json);
				})['catch'](function(error){
					reject(error);
		...
		deleteRealm: function(params){
			return new Promise(function(resolve, reject) {
				...
				b.$.doDelete(url).then(function(json){
					services.auth.updateLastActiveTimestamp();
					resolve(json);
				})['catch'](function(error){
					reject(error);
		...
		getRealmUser: function(params){
			return new Promise(
				function(resolve, reject) {
					...
					b.$.getJSON(url).then(function(json){
						services.auth.updateLastActiveTimestamp();
						//remove bridgeit.user permisisons as they're default
						//and will cause errors on updates
						var permissions = [];
						for( var i = 0 ; i < json.user.permissions.length ; i++ ){
							if( json.user.permissions[i].indexOf('bridgeit.user') !== 0 ){
								permissions.push(json.user.permissions[i]);
							}
						}
						json.user.permissions = permissions;
						resolve(json.user);
					})['catch'](function(error){
						reject(error);
					});
