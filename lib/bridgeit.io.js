		...
		connect: function(params){
			return new Promise(function(resolve, reject) {
				...
				function connectCallback(){
					...
					var millisUntilTimeoutExpires = timeoutMillis - inactiveMillis - timeoutPadding;
					...
					if( millisUntilTimeoutExpires > 0 ){
						...
						//if the time remaining before expiry is less than the session timeout
						...
						if( timeoutMillis > millisUntilTimeoutExpires ){
							services.auth.refreshAccessToken().then(function(){
								//set the the next timeout check to either just before the session timeout expires or just before the token timeout expires
								var nextTimeout = millisUntilTimeoutExpires < services.auth.getExpiresIn() ? 
									( millisUntilTimeoutExpires - timeoutPadding ) : (services.auth.getExpiresIn() - timeoutPadding );

								if( nextTimeout < 0 ){
									nextTimeout = services.auth.getTimeRemainingBeforeExpiry();
								}
								var cbId = setTimeout(connectCallback, nextTimeout);
								b.setSessionStorageItem(btoa(RELOGIN_CB_KEY), cbId);
								console.log( new Date().toISOString() + ' -----------> bridgeit.io.auth.connect: setting next connection check to ' + nextTimeout / 1000 / 60 + ' mins, expiresIn: ' +
    ...
    createCustomEvents: function(params){
      return new Promise(
        function(resolve, reject) {
          ...
          runningIndex += 1;
          eventIndex=0;
          services.event._eventRecursion(resolve,reject,url,runningIndex);
        }
      );
    },
    /**
            ...
            if(index === runningIndex) {
              ...
              b.$.post(url, eventArray[eventIndex]).then(function (response) {
                ...
                if (eventIndex === eventArray.length - 1) {
                  ...
                  eventIndex += 1;
                  services.event._eventRecursion(resolve, reject, url, index);
                }
              })['catch'](function (error) {
                reject(error);
              });
