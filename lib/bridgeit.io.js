...
(function(b) {
	...
	services.auth = {
		...
		connect: function(params){
          ...
          console.log('time since last activity: ' + (( now - services.auth.getLastActiveTimestamp())/1000/60) + ' minutes.');
					if( ( now - services.auth.getLastActiveTimestamp()) < timeoutMillis){
						...
						var timeRemaining = connectionTimeoutMillis - (now - services.auth.getLastActiveTimestamp());
            if( (connectSettings.connectionTimeout * 1000 * 60 ) > services.auth.getTimeRemainingBeforeExpiry()){
              services.auth.refreshAccessToken();
              //setTimeout(connectCallback,  (settings.connectionTimeout - 1) * 60 * 1000);
              //initConnectCallback();
              setTimeout(connectCallback, timeRemaining);
						}
						else{
							console.log( new Date().toISOString() + ' bridgeit connect: setting callback for ' + timeRemaining/60/1000 + ' minutes');
							setTimeout(connectCallback, timeRemaining);
